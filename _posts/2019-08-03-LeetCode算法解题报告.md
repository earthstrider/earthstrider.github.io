...
假设A总是先拿石子，B在其之后，res[i]表示n=i时的结果。因为双方都有三种取法，当n=i时候、res[i]与res[i-1]、res[i-2]、res[i-3]有关系，要想res[i]值是true，那么res[i-1]、res[i-2]以及res[i-3]中至少一个值是false，即将问题划分为两部分，首先A拿1个或者2个或者3个石子，接着B拿石子（不需要确定个数），结果A拿最后一个石子，符合条件。
