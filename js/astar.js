class Astar {
  constructor(row = 20, col = 20) {
	...
	init() {
		...
		for (let i = 0; i < this.row; i++) {
			for (let j = 0; j<this.col; j++){
				if (this.myMap[i][j]==='4' || this.myMap[i][j]==='5')
					this.myMap[i][j] = '0';
			}
		}
		this.drawMap();
	...
	navigate() {
        ...
        do {
			...
			// set closeList
			for (let i=1; i<this.closeList.length; i++){
				let point = this.closeList[i];
				this.myMap[point.x][point.y] = '5';
			}
